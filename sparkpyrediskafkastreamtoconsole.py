...
spark = SparkSession.builder.appName('redis-server-to-spark').getOrCreate()
...
emailAndBirthDayStreamingDF = spark \
    .sql('SELECT email, birthDay FROM Customer WHERE email IS NOT NULL AND birthDay IS NOT NULL')
